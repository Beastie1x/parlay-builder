...
def normalize_team(t:str)->str: return t.upper().replace(" ","")
...
if st.button("Build Parlay", type="primary"):
    with st.spinner("Fetching lines and building model…"):
        ...
        if df is None or df.empty:
            st.warning(f"No lines for {platform}. Use Secrets or upload CSV."); st.stop()
        try:
            pbp = fetch_pbp(2024)
        ...
        if not picks: st.warning("No modeled markets found."); st.stop()
        import numpy as np, pandas as pd
        picks_df = pd.DataFrame(picks).sort_values("edge", ascending=False).head(legs)
        st.success("Done!"); st.dataframe(picks_df, use_container_width=True)
        st.markdown("### Recommended Slip")
        for _, r in picks_df.iterrows():
            st.markdown(f"**{r['player']} — {r['market']} {r['line']}**  
"
                        f"Model mean: **{r['mean']:.1f}**  
"
                        f"Edge vs fair: **{r['edge']:+.2%}**  
"
                        f"{r['reason']}"); st.divider()
...
- Model mean: {r['mean']:.1f}
- Edge: {r['edge']:+.2%}
{r['reason']}"
            for _, r in picks_df.iterrows()
        )
        st.download_button("Download Markdown", md, "parlay.md", "text/markdown")
